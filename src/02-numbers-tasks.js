...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 1) {
    ...
    if (+numArr[numArr.length - 3] > 4) {
      numArr[numArr.length - 3] = +numArr[numArr.length - 3] + 1;
    }
    return +numArr.join('');
  } if (pow === 3) {
    numArr[numArr.length - 1] = '0';
    numArr[numArr.length - 2] = '0';
    numArr[numArr.length - 3] = '0';

    if (+numArr[numArr.length - 4] > 4) {
      numArr[numArr.length - 4] = +numArr[numArr.length - 4] + 1;
    }
    return +numArr.join('');
  }
  return num;
...
function toNumber(value, def) {
  if (typeof parseInt(value, 10) === 'number' && !Number.isNaN(Number(value)) && (value)) return parseInt(value, 10);
  return def;
}
